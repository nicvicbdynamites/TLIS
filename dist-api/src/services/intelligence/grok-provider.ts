/**
 * Grok Provider — xAI Grok (OpenAI-compatible API). Priority 5.
 */
...
export class GrokProvider extends BaseAiProvider {
  ...
  readonly models      = ["grok-beta"] as const;
  readonly priority    = 5;
  ...
  async generate(prompt: string, opts?: GenerateOpts): Promise<GenerateResult> {
    if (!this.isConfigured()) throw new Error("XAI_API_KEY not configured");
    const start = Date.now();
    ...
    if (!resp.ok) { const t = await resp.text().catch(() => ""); throw new Error(`Grok ${resp.status}: ${t.slice(0, 120)}`); }
  ...
  async analyze(content: string, task: string, opts?: GenerateOpts): Promise<AnalyzeResult> {
    ...
    return { analysis: result.text, confidence: 80, provider: this.id, model: result.model, latencyMs: Date.now() - start };
  }
