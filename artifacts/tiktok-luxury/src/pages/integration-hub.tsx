...
import { useIntelligenceStatus, type ScheduledTask } from "@/lib/intelligence-service";
import {
  useIntegrationRegistry, testIntegrationConnection, integrationStatusToDisplay,
  type IntegrationEntry, type IntegrationConnectionState, type IntegrationTestResult,
...
// ── Overview / AI Providers — live formatting helpers ─────────────────────

function toConnStatus(status: IntegrationConnectionState): ConnStatus {
  const d = integrationStatusToDisplay(status);
  if (d === "Configure") return "Disconnected";
  if (d === "Fallback")  return "Error";
  return d;
}
...
function formatRelativeFuture(iso?: string): string {
  ...
  return `Est. ${Math.ceil(mins / 60)}h`;
}
...
const AI_SECRET_ENV_MAP: Record<string, string> = {
...
function toAIProvider(entry: IntegrationEntry): AIProvider {
  const status = toConnStatus(entry.status);
  const keyConfigured = entry.status !== "unconfigured";
  const lastResponse =
    entry.status === "connected"    ? `200 OK${entry.avgResponseTimeMs ? ` · ${entry.avgResponseTimeMs}ms` : ""}`
    : entry.status === "rate_limited" ? "Rate limited"
    : entry.status === "error"        ? "Error"
    : entry.status === "initializing" ? "Checking…"
    : "—";
  return {
    id:            entry.id,
    name:          entry.name,
    icon:          AI_ICON_MAP[entry.id] ?? Bot,
    status,
    keyConfigured,
    keyName:       AI_SECRET_ENV_MAP[entry.id] ?? `${entry.id.toUpperCase()}_API_KEY`,
    lastRequest:   formatRelativeTime(entry.lastHealthCheckAt),
    lastResponse,
    health:        entry.status === "connected" ? 100 : entry.status === "rate_limited" ? 60 : entry.status === "initializing" ? 30 : 0,
    model:         entry.apiVersion ?? "—",
  };
}

// ── Background Jobs — derived from live scheduler tasks ────────────────────

function bucketiseTask(task: ScheduledTask, now: number): JobStatus {
  if (task.lastStatus === "error") return "failed";
  if (!task.lastRunAt) return "waiting";
  if (task.nextRunAt && new Date(task.nextRunAt).getTime() <= now) return "running";
  return "completed";
}

function toJobRow(task: ScheduledTask, bucket: JobStatus): { job: string; detail: string; time: string } {
  switch (bucket) {
    case "failed":
      return { job: task.name, detail: task.lastError ?? "Unknown error", time: formatRelativeTime(task.lastRunAt) };
    case "waiting":
      return { job: task.name, detail: `Awaiting first run · every ${formatInterval(task.intervalMs)}`, time: formatRelativeFuture(task.nextRunAt) };
    case "running":
      return { job: task.name, detail: `Due now · every ${formatInterval(task.intervalMs)}`, time: "Due now" };
    default:
      return { job: task.name, detail: `Run #${task.runCount} · next in ${formatRelativeFuture(task.nextRunAt).replace("Est. ", "")}`, time: formatRelativeTime(task.lastRunAt) };
  }
}
...
// ── Background Jobs — populated at runtime from live scheduler tasks ───────
// (see `bucketiseTask` / `toJobRow` helpers and the `bgJobs` memo below)
...
export default function IntegrationHub() {
  ...
  const { integrations, summary }                                  = useIntegrationRegistry();
  const { scheduler }                                               = useIntelligenceStatus();

  const aiProviders = useMemo(
    () => integrations.filter(i => i.category === "ai").map(toAIProvider),
    [integrations],
  );
  ...
  const overview = useMemo(() => {
    ...
    const lastHealthCheckAt = integrations.reduce<string | undefined>((latest, i) => {
      if (!i.lastHealthCheckAt) return latest;
      if (!latest || i.lastHealthCheckAt > latest) return i.lastHealthCheckAt;
      return latest;
    }, undefined);
    const now = Date.now();
    const activeJobs = scheduler.filter(t => bucketiseTask(t, now) === "running").length;
    return {
      connected,
      disconnected: Math.max(total - connected, 0),
      apiHealth:    total > 0 ? Math.round((connected / total) * 100) : 0,
      lastSync:     formatRelativeTime(lastHealthCheckAt),
      activeJobs,
    };
  }, [integrations, summary, scheduler]);

  const bgJobs = useMemo(() => {
    const now = Date.now();
    const buckets: Record<JobStatus, { job: string; detail: string; time: string }[]> = {
      running: [], waiting: [], completed: [], failed: [],
    };
    for (const task of scheduler) {
      const bucket = bucketiseTask(task, now);
      buckets[bucket].push(toJobRow(task, bucket));
    }
    return buckets;
  }, [scheduler]);
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-8">
      ...
      <div className="luxury-card p-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <span className="ml-auto text-[10px] font-mono text-muted-foreground/50">
            {aiProviders.filter(p => p.status === "Connected").length}/{aiProviders.length} Connected
          </span>
        </div>
          ...
          {aiProviders.map(provider => (
            ...
            >
              ...
              <button
                ...
                disabled={!provider.keyConfigured || testingProvider === provider.id}
                    ...
                    {r.success ? (
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <p className="font-mono text-foreground truncate">{r.message ?? provider.model}</p>
                        </div>
                        <div>
                      ...
                      <p className="text-red-400/80 leading-relaxed">{r.message ?? "Connection failed"}</p>
                    )}
                  </div>
                );
              })()}
      ...
      <div className="luxury-card p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
          {[
            { label: "Running",   value: bgJobs.running.length,   color: "text-primary"     },
            { label: "Waiting",   value: bgJobs.waiting.length,   color: "text-amber-400"   },
            { label: "Completed", value: bgJobs.completed.length, color: "text-emerald-400" },
            { label: "Failed",    value: bgJobs.failed.length,    color: "text-red-400"     },
          ].map(s => (
