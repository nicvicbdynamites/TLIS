/**
 * Mistral Provider — Mistral AI (mistral-small). Priority 6.
 * Uses native fetch against the Mistral Chat Completions API.
 */
...
export class MistralProvider extends BaseAiProvider {
  ...
  readonly models      = ["mistral-small-latest", "mistral-tiny"] as const;
  readonly priority    = 6;
  ...
  async generate(prompt: string, opts?: GenerateOpts): Promise<GenerateResult> {
    if (!this.isConfigured()) throw new Error("MISTRAL_API_KEY not configured");
    const start = Date.now();
    ...
    if (!resp.ok) { const t = await resp.text().catch(() => ""); throw new Error(`Mistral ${resp.status}: ${t.slice(0, 120)}`); }
  ...
  async analyze(content: string, task: string, opts?: GenerateOpts): Promise<AnalyzeResult> {
    ...
    return { analysis: result.text, confidence: 76, provider: this.id, model: result.model, latencyMs: Date.now() - start };
  }
